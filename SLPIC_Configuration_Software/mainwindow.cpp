...
void MainWindow::on_actionConnect_triggered()
{
    if(m_pConnection==Q_NULLPTR)
    {
        ...
        connect(m_pConnection,SIGNAL(serialPortOpened(QSerialPort* )),this,SLOT(on_serialPortOpened(QSerialPort*)));
...
void MainWindow::on_serialPortOpened(QSerialPort *serialPort)
{
...
{
    static int count=0;
    static int countDI=0;
    if(m_AI_Packetes_ToSend.length()>0)
    {
        if(m_pSerialPort->isOpen())
        {
            //m_pSerialPort->write(m_AI_Packetes_ToSend.at(count).Packet());
            if(m_Fram>=255)
            {
                m_Fram=0;
            }
            m_AI_Packetes_ToSend[count].setFrame(m_Fram++);

            //m_AI_Packetes_ToSend[count].Packet();
            m_pSerialPort->write( m_AI_Packetes_ToSend.at(count).m_Packet);
            count++;
            if(count>=m_AI_Packetes_ToSend.length())
            {
                count=0;
            }

            m_DI_Packetes_ToSend[countDI].setFrame(m_Fram++);
            m_pSerialPort->write( m_DI_Packetes_ToSend.at(countDI).m_Packet);
            countDI++;
            if(countDI>=m_DI_Packetes_ToSend.length())
            {
                countDI=0;
            }
