...
bool MainWindow::checkDataBaseIsNotEmpty()
{
    bool res=false;

    //数据库表名，全局变量
    static QString TableNames[]={"AI","DI","DO","Other","SerialPort"};
    if(m_FilePath==tr(""))
    {

        //项目为空时也需要重新建立一个项目（数据库实力）
        res=true;
        return res;
    }
    ...
    if(m_DataBase.Open(m_FilePath))
    {
         ...
         for(int i=0;i<sizeof(TableNames)/sizeof(TableNames[0]);i++)
         {
             ...
             {
                 QMessageBox::warning(this,tr("WARNING"),model.lastError().text());
...
void MainWindow::on_actionUpload_triggered()
{
    ...
    //检查当前项目是否为空项目。
    if(checkDataBaseIsNotEmpty())
    {
        QMessageBox::StandardButton ret;
        ret=QMessageBox::information(this,tr("WARNING"),tr("为了当前项目不丢失，新建一个新项目"));
    ...
    if(m_pSerialPort->isOpen())
    {
        // 要读取的通道AI, DI,DO,serialport,other信息
        //AI
        ...
        m_SendArray.clear();
        m_RecieveArray.clear();
        for(int i=0;i<MAX_AI_ChannelNum;i++)
        {
            DataPacketToSend ai_Packet(DataPacketToSend::read);
            DataPacketToSend::StandPacketRelevant standRelevant;
            //channel:1..8
            standRelevant.m_ChannelNum=i+1;
            //ProjectName:AI=0x01
            standRelevant.m_ProjectName=0x01;
            standRelevant.m_ValuePacket.append(0x01);
            standRelevant.m_ValuePacket.append(0x03);
            standRelevant.m_ValuePacket.append(0x04);
            standRelevant.m_ValuePacket.append(0x05);
            standRelevant.m_ValuePacket.append(0x06);
            standRelevant.m_ValuePacket.append(0x07);
            standRelevant.m_ValuePacket.append(0x08);
            standRelevant.m_ValuePacket.append(0x09);
            standRelevant.m_ValuePacket.append(0x0A);
            standRelevant.m_ValuePacket.append(0x0B);
            standRelevant.m_ValuePacket.append(0x0C);

            m_SendArray.append(ai_Packet.AssemblePacket(standRelevant));

        }

        //di
        for(int i=0;i<MAX_DI_ChannelNum;i++)
        {
            DataPacketToSend di_Packet(DataPacketToSend::read);
            DataPacketToSend::StandPacketRelevant standRelevant;
            //channel:1..8
            standRelevant.m_ChannelNum=i+1;
            //ProjectName:AI=0x01
            standRelevant.m_ProjectName=0x02;
            standRelevant.m_ValuePacket.append(0x01);
            standRelevant.m_ValuePacket.append(0x0D);
            m_SendArray.append(di_Packet.AssemblePacket(standRelevant));
        }

        //do
        for(int i=0;i<MAX_DO_ChannelNum;i++)
        {
            DataPacketToSend do_Packet(DataPacketToSend::read);
            DataPacketToSend::StandPacketRelevant standRelevant;
            //channel:1..8
            standRelevant.m_ChannelNum=i+1;
            //ProjectName:AI=0x01
            standRelevant.m_ProjectName=0x03;
            standRelevant.m_ValuePacket.append(0x01);
            //standRelevant.m_ValuePacket.append(0x0D);
            m_SendArray.append(do_Packet.AssemblePacket(standRelevant));
        }

        //sp
        for(int i=0;i<MAX_SP_ChannelNum;i++)
        {
            DataPacketToSend sp_Packet(DataPacketToSend::read);
            DataPacketToSend::StandPacketRelevant standRelevant;
            //channel:1..4
            standRelevant.m_ChannelNum=i+1;
            //ProjectName:AI=0x01
            standRelevant.m_ProjectName=0x04;
            standRelevant.m_ValuePacket.append(0x01);
            standRelevant.m_ValuePacket.append(0x02);
            standRelevant.m_ValuePacket.append(0x03);
            standRelevant.m_ValuePacket.append(0x04);
            standRelevant.m_ValuePacket.append(0x05);
            standRelevant.m_ValuePacket.append(0x06);
            ...
            //standRelevant.m_ValuePacket.append(0x0D);
            m_SendArray.append(sp_Packet.AssemblePacket(standRelevant));
        }

        //other
        for(int i=0;i<MAX_Ohter_ChannelNum;i++)
        {
            DataPacketToSend other_Packet(DataPacketToSend::read);
            DataPacketToSend::StandPacketRelevant standRelevant;
            //channel:1..4
            standRelevant.m_ChannelNum=i+1;
            //ProjectName:AI=0x01
            standRelevant.m_ProjectName=0x05;
            standRelevant.m_ValuePacket.append(0x01);
            standRelevant.m_ValuePacket.append(0x02);
            standRelevant.m_ValuePacket.append(0x03);
            standRelevant.m_ValuePacket.append(0x04);
            ...
            standRelevant.m_ValuePacket.append(0x0a);
            standRelevant.m_ValuePacket.append(0x0d);
            standRelevant.m_ValuePacket.append(0x0e);
            standRelevant.m_ValuePacket.append(0x0f);

            //standRelevant.m_ValuePacket.append(0x0D);
            m_SendArray.append(other_Packet.AssemblePacket(standRelevant));
        }
