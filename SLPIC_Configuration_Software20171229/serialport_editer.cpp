...
void SerialPort_Editer::InitBaseTab()
{
    ...
    ui->Linklable->setText(tr("<a href=\"#\">获取平台参数"));
     ui->Linklable_2->setText(tr("<a href=\"#\">获取平台参数"));
    connect(ui->Linklable,SIGNAL(linkActivated(QString)),this,SLOT(on_linkActivated(QString)));
    connect(ui->Linklable_2,SIGNAL(linkActivated(QString)),this,SLOT(on_linkActivated_2(QString)));
...
void SerialPort_Editer::getSubProjectNameTable(SubProjectNameTable *pSubProjectNameTable)
{
...
{
    //int i=m_mapFtManufactures.keys().length();
    if(m_mapFtManufactures.keys().length()==0)
    {
        if(m_pSubProjectNameTable!=Q_NULLPTR)
        {
             m_mapFtManufactures= m_pSubProjectNameTable->getFtManufactureMap();
    ...
    else
    {
        ...
        int manufactureId=items[10].toInt();
        QString ftManufactureName=m_mapFtManufactures[manufactureId];
        ui->Manufacture_Name->setText(ftManufactureName);
...
void SerialPort_Editer::on_linkActivated(QString s)
{
    ...
    connect(m_pSubProjectNameTable,SIGNAL(setSP_Edit_Info(SubProjectNameBean,EditPageNum)),this,SLOT(on_setSP_Edit_Info(SubProjectNameBean,EditPageNum)));
    //发送信号：告知系统信息表属于：AI，DI，DO...的哪一种
    emit linkSubProjectName(SerialPortPage_Sub_ProjName);

    m_pSubProjectNameTable->exec();
}

void SerialPort_Editer::on_linkActivated_2(QString s)
{
    if(m_pSubProjectNameTable==Q_NULLPTR)
    {
        QMessageBox::warning(this,tr("WARNING"),tr("未找到系统配置信息"));
    ...
    connect(m_pSubProjectNameTable,SIGNAL(setSP_Edit_Info(SubProjectNameBean,EditPageNum)),this,SLOT(on_setSP_Edit_Info(SubProjectNameBean,EditPageNum)));
    //发送信号：告知系统信息表属于：AI，DI，DO...的哪一种
    emit linkSubProjectName(SerialPortPage_FT_Manufacture);

    m_pSubProjectNameTable->exec();

}

void SerialPort_Editer::on_setSP_Edit_Info(SubProjectNameBean bean, EditPageNum page)
{
    if(page==SerialPortPage_Sub_ProjName)
    {
        ui->SubPrjName->setText(bean.m_Filed);
        ui->SubPrjNameID->setText(QString::number(bean.m_SeriesNumber,10));
    }
    else if(page==SerialPortPage_FT_Manufacture)
    {
        ui->Manufacture->setText(QString::number(bean.m_SeriesNumber));
        ui->Manufacture_Name->setText(bean.m_Filed);
    }
